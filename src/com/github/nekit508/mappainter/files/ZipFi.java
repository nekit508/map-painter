package com.github.nekit508.mappainter.files;
...
public class ZipFi extends Fi {
    ...
    public Fi child(String name){
        ...
        String nextChildren = ind != -1 ? name.substring(0, ind) : name;
        String lastPart = (ind != name.length()-1 && ind != -1) ? name.substring(ind+1) : null;

        if (nextChildren.equals(".."))
            return parent().child(lastPart);

        for(ZipFi child : children){
            if(child.name().equals(nextChildren)){
                return lastPart == null ? child : child.child(lastPart);
            }
        }
